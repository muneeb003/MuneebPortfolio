...
function ChatWindow({ about }: { about: About }) {
  ...
  const sendUserMsg = async (text: string) => {
    ...
    const userMsg: ChatMsg = { id: nextId.current++, from: "user", text: text.trim() };
    ...
    try {
      ...
      setMessages((m) => [
        ...m,
        { id: nextId.current++, from: "bot", text: "Something went wrong on my end 😅 Try the Contact section to reach me directly!" },
      ]);
    } finally {
      setDisabled(false);
    ...
    <div className="flex flex-col rounded-[2rem] border border-zinc-700/60 bg-zinc-900 overflow-hidden shadow-2xl h-[520px]">
      ...
      <div className="flex items-center gap-3 px-5 py-3 border-b border-zinc-800">
        <div className="relative w-9 h-9 rounded-full overflow-hidden bg-zinc-800 shrink-0">
          {about.avatar_url && (
            <Image src={about.avatar_url} alt="Muneeb" fill sizes="36px" className="object-cover" />
          )}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-zinc-100 leading-none">Muneeb</p>
          <p className="text-xs text-emerald-400 mt-0.5 flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block" />
            online
          </p>
        </div>
        <span className="text-zinc-600 text-xs bg-zinc-800 px-2 py-1 rounded-full">chatbot</span>
      </div>
      ...
      >
        <p className="text-center text-[10px] text-zinc-600 mb-3 select-none">Today</p>
            ...
            >
              ...
              {msg.from === "bot" && (
                <div className="relative w-6 h-6 rounded-full overflow-hidden bg-zinc-800 shrink-0 mb-0.5">
                  {about.avatar_url && (
                    <Image src={about.avatar_url} alt="" fill sizes="24px" className="object-cover" />
                  )}
                </div>
              )}
...
function StatCounter({ end, suffix = "" }: { end: number; suffix?: string }) {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(([e]) => {
      ...
      if (!e.isIntersecting) { started.current = false; setCount(0); }
    }, { threshold: 0.6 });
    obs.observe(el);
...
const SOCIAL_META: Record<string, { icon: string; color: string; border: string; desc: string }> = {
  GitHub:   { icon: "⌥", color: "hover:bg-zinc-700/60",   border: "hover:border-zinc-500",      desc: "See my work"    },
  LinkedIn: { icon: "💼", color: "hover:bg-sky-500/10",    border: "hover:border-sky-500/40",    desc: "Let's connect"  },
  Twitter:  { icon: "𝕏",  color: "hover:bg-zinc-700/60",   border: "hover:border-zinc-500",      desc: "Say hi"         },
  Email:    { icon: "✉",  color: "hover:bg-indigo-500/10", border: "hover:border-indigo-500/40", desc: "Drop a message" },
};
  ...
  return (
    <section id="about" className="py-24 px-6 max-w-5xl mx-auto">
      <FadeUp className="mb-12">
        <SplitHeading className="text-3xl font-bold text-zinc-100">About me</SplitHeading>
      </FadeUp>
      ...
      <div className="grid md:grid-cols-[1.1fr_1fr] gap-10 items-start">
        ...
        >
          <motion.div
            whileHover={{ y: -4, boxShadow: "0 24px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(99,102,241,0.2)" }}
            transition={{ type: "spring", stiffness: 200, damping: 20 }}
            className="relative h-[520px] rounded-3xl overflow-hidden flex flex-col justify-between p-7"
            style={{
              background: "linear-gradient(145deg, #18181b 0%, #0f0f11 50%, #18181b 100%)",
              boxShadow: "0 12px 40px rgba(0,0,0,0.4), 0 0 0 1px rgba(63,63,70,0.5)",
            }}
          >
            {/* Corner glow */}
            ...
            <div className="relative z-10">
              <h3 className="text-4xl font-bold text-zinc-100 leading-none tracking-tight">Muneeb</h3>
              <p className="text-indigo-400 font-medium mt-2 text-sm">Full Stack Developer</p>
              {about.bio_short && (
                <p className="text-xs text-zinc-500 mt-3 leading-relaxed max-w-[220px]">{about.bio_short}</p>
              )}
            </div>
            ...
            <div className="relative z-10 flex gap-8">
              {[
                { end: 3,  suffix: "+", label: "Years exp",       accent: "text-indigo-400" },
                { end: 20, suffix: "+", label: "Projects shipped", accent: "text-purple-400" },
              ].map(({ end, suffix, label, accent }) => (
                <div key={label}>
                  <p className="text-3xl font-bold text-zinc-100 leading-none tabular-nums">
                    <StatCounter end={end} /><span className={accent}>{suffix}</span>
                  </p>
                  <p className="text-[10px] text-zinc-600 mt-1.5 uppercase tracking-wider">{label}</p>
                </div>
              ))}
            </div>
            ...
            <div className="relative z-10 space-y-1.5">
              {about.location_label && (
                <p className="text-xs text-zinc-500 flex items-center gap-2">
                  <span className="text-zinc-600">📍</span>{about.location_label}
                </p>
              )}
              {about.email && (
                <p className="text-xs text-zinc-500 flex items-center gap-2">
                  <span className="text-zinc-600">✉</span>{about.email}
                </p>
              )}
            </div>
            ...
            <div className="relative z-10 flex items-center justify-between">
              <div className="flex gap-2">
                {socials.map((s) => {
                  const meta = SOCIAL_META[s.label] ?? { icon: "→", color: "", border: "", desc: "" };
                  return (
                    <motion.a
                      key={s.label}
                      href={s.href!}
              ...
              <span className="text-[10px] text-zinc-700 font-mono">muneeb.dev</span>
            </div>
          </motion.div>
        </motion.div>
      </div>
